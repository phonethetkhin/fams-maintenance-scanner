...
public class AssetListActivity extends AppCompatActivity implements ScanListener {
    ...
    private static final int SCAN_KEY = 0;        // Scan key default     // Flag to start scanning
    ...
    public static AssetListActivity instance = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        location = findViewById(R.id.selected_location);
        location.setText(helper.getLocationById(locationId).getCode() + " - " + helper.getLocationById(locationId).getName());

        assetList = helper.getAssetByLocation(locationId);

        recyclerView = findViewById(R.id.location_asset_list);
        adapter = new AssetListAdapter(assetList, getApplicationContext());
    ...
    ServiceConnection serviceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            ...
            scanService.setOnScanListener(AssetListActivity.this);
        }
    };
...
//        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.action_qr_scan:
                new IntentIntegrator(AssetListActivity.this).setCaptureActivity(QRCodeScannerActivity.class).initiateScan();
                break;
            default:
        }
    ...
    public void showResultDialogue(final String result) {
        prefs = getSharedPreferences(prefName, MODE_PRIVATE);
        int locationId = prefs.getInt("locationId", 0);
        List<AssetBean> assetList = helper.getAssetByLocation(locationId);
    ...
    public void onEvent(Event event) {
        ...
        if (event.equals(Event.Request_Data)) {
            ...
            scheduleList = helper.getAllScheduleObjects(fixedAsset.getId());
            maintenanceList = helper.getAllMaintenanceObjects(fixedAsset.getId());

            if (scheduleList.size() > 0 || maintenanceList.size() > 0) {
                ...
                Intent intent = new Intent(AssetListActivity.this, ScanActivity.class);
